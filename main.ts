 ...
 * — when given a prompt — creates one agent, feeds it the prompt, waits for the
 * turn to settle, and exits. With no prompt it boots the tree and exits (a
 * readiness check / sidecar-style host start).
 *
 * Usage:
...
import type { AgentOptions } from '@cos/types'
...
if (prompt === undefined || prompt === '') {
  // Readiness / host mode: the tree is booted and parked. In a real deployment
  // this process would serve the sidecar; here it exits cleanly so callers can
  // treat a non-zero exit as a boot failure.
  await ctx.fiber.dispose()
  process.exit(0)
}

// The agent's route must be explicit (no guessing in production). Resolve it
// from the mounted adapter and its first advertised model, so a bare invocation
// still works while remaining explicit at the call site.
...
const lastAssistant = agent.session.events
  .filter((event): event is Extract<import('@cos/types').SessionEvent, { type: 'assistant/message' }> => event.type === 'assistant/message')
  .at(-1)
