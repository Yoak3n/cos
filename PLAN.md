<!-- 编辑注（非计划原文）：本仓库（E:\Project\RustProject\cos，当前 crate 名 cos）即本计划的 "cos/" 工作区根，§2 的目录树直接映射到本仓库根；JS 语义权威参考位于 E:\GitVault\deepseek-harness。 -->

# cos 实施计划（A 形态主干 → B 形态生态）

## 0. 目标与非目标

**目标**：用 Rust 再实现插件化主干（借鉴 dsh 的思想）—— "一切皆插件"的语义（Context 服务仓库、inject/provide 依赖、事件扩展、可逆效果、会话日志为唯一事实源、turn/step 主循环），启动/重启时由 cordis.yml 装配插件树。
...
| 能力接缝三件套（Definition/Provider/Consumer） | 每能力 = cos-<cap> 定义 trait + cos-<cap>-local 实现 + plugin-tool-* 消费 |
...
| Rust crate | JS 参考文件 |
| --- | --- |
| cos-core | vendor/cordis/src/{context,registry,fiber,events,reflect}.ts、packages/core/scope/src/index.ts |
| cos-loader | vendor/loader/src/config/{entry,tree,group}.ts、packages/boot/app-boot/src/profile.ts |
| cos-session | packages/core/session/src/ |
| cos-agent / cos-agent-loop | packages/core/agent/src/types.ts、packages/core/agent-loop/src/{agent,index,tool-calls}.ts |
| cos-tools | packages/core/tools/src/、docs/tool-execution-pipeline.md |
| cos-system-prompt | packages/core/system-prompt/src/ |
...
cos/  (Cargo workspace)
├── crates/
│   ├── cos-core/          # Context、服务注册表、事件总线、Plugin trait、Fiber/effect、scope
│   ├── cos-loader/        # cordis.yml 解析 → 工厂解析 → 拓扑排序 → 挂载
│   ├── cos-session/       # SessionEvent 追加日志、derive_messages、JSONL 持久化
│   ├── cos-llm/           # LLM 接缝：Message/ContentBlock、LlmAdapter trait、stream
│   ├── cos-llm-mock/      # 确定性脚本化 mock 适配器（测试与回放）
│   ├── cos-tools/         # 工具注册表 + 执行管线（pre/guards/execute/post）
│   ├── cos-system-prompt/ # prompt 段 + 工具 schema 装配
│   ├── cos-agent/         # Agent trait、注册表、Inbox（接缝）
│   ├── cos-agent-loop/    # turn/step 驱动器（实现 cos-agent）
│   ├── cos-shell/         # shell 接缝 + local 实现 + tool-bash（可推迟到 P6）
│   └── cos-contract/      # (P7 起) 版本化契约 crate —— B 形态的地基
...
依赖方向铁律（照 dsh）：plugins/* 和 cos-agent-loop 只能依赖各接缝的 Definition crate（cos-llm/cos-tools/cos-agent/cos-shell），不得依赖具体 Provider 或 cos-agent-loop 本身；cos-core 不依赖任何上层 crate。
...
// cos-session —— 封闭枚举 + Custom 逃生舱（决策 D4）
...
// cos-agent-loop：实现该 trait，注册为 ctx.agents 的工厂
...
| D6 | scope 是否现在就做 | 先单 agent / 现在就做事件按 scope 路由 | 现在就做（cos-scope 那套 ScopeKey + 父链 + 事件向上流），子 agent/subagent 是主干的一部分 |
...
workspace + 各 crate 空壳 + cos-core 内 Plugin/Context/Fiber/EventBus trait 初稿 + 决策文档 docs/decisions.md
...
cos-session：SessionEvent 追加日志、seq、derive_messages()、JSONL 持久化（session/flush 同款 checkpoint 可后置）
cos-llm：Message/ContentBlock、LlmAdapter trait（stream(request) -> Stream<Chunk>）、cos-llm-mock：脚本化确定性回复（按输入哈希或序号选择预设输出）

✅ 验收：derive_messages 投影正确性单测；mock 适配器可流式产出 chunk；持久化 → 重载 → 日志逐字节一致

### P4 agent + agent-loop（L，主干核心）

cos-agent：Agent trait、注册表、Inbox（next_turn/next_step）、with_initiator（task_local 因果链）
cos-agent-loop：turn/step 状态机照 agent.ts —— wake_driver → kick → turn → pre_step(waterfall) → step(build_request → stream → assistant 事件 → 工具) → turn_end；每步先写日志再行动
...
cos-tools：注册表 + pre-execute/execute/post-execute waterfall + tool/call、tool/result 事件先写日志再执行；cos-system-prompt：prompt 段装配 + 工具 schema 收集
...
app/ 可执行：cos --config cordis.yml 启动、--dump-config、优雅退出（全 fiber 逆序卸载）
cos-shell 接缝 + local + plugin-bash（v1 简化：前台执行、无后台 job、无 sandbox 后端）
不变量注册表（cos-invariants，同 dsh ctx.invariants）：模型可见 ⟺ 已记录、事件 seq 单调等

✅ 验收：A 形态 DoD 全绿（见 §7）；cos --dump-config 输出与装载一致；卸载顺序日志可审计

### P7 B 形态准备（契约冻结，M）

提炼 cos-contract：版本化（semver）契约 crate；接缝 trait 做对象安全审计（消除公开泛型方法）；服务方法"小而窄"审计（参数/返回值可 JSON 序列化）
...
✅ 验收：cos-contract 独立发布构建通过；所有接缝 trait 过 object_safe 检查；B-ABI 文档评审通过
