...
use cos_session::{
    SESSION_FORMAT_VERSION, SessionEvent, SessionEventData, SessionHeader, load_jsonl, save_jsonl,
};
use cos_shell::provide_local_shell;
use cos_system_prompt::PromptSections;
#[cfg(feature = "plugin-opencode")]
use plugin_opencode::OPENCODE_KIND;
use thiserror::Error;
pub mod plugins;
pub mod repl;
pub mod rpc;
...
pub async fn assemble(config: &RunConfig) -> Result<Assembled, AppError> {
    ...
    if let Some(cfg) = &config.llm {
        ...
        #[cfg(not(feature = "plugin-opencode"))]
        {
            let _ = cfg;
