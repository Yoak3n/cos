//! dsh-agent-loop —— turn/step 驱动器，实现 dsh-agent（P4，主干核心）。
//!
...
use dsh_core::{Context, ScopeKey, ScopeTarget};
use dsh_llm::{
    AssistantMessage, ChunkDelta, ContentBlock, LlmAdapter, LlmRequest, ToolCall, UserMessage,
};
use dsh_session::{AbortCause, RequestHeader, Session, SessionEventData, TurnEndReason};
...
impl AgentCore {
    ...
    fn spawn_driver(&self) {
        ...
        tokio::spawn(dsh_agent::with_initiator(agent, async move {
            kick(core).await;
        }));
    }
    ...
    async fn run_step(
        ...
        let tool_registry = self.agent_ctx.get::<dsh_tools::ToolRegistry>().ok();
        ...
        let system = self
            .agent_ctx
            .get::<dsh_system_prompt::PromptSections>()
            .ok()
        ...
        for call in &tool_calls {
            ...
            let outcome = match &tool_registry {
                ...
                None => dsh_tools::ToolOutcome::error(
            ...
            self.session.append(SessionEventData::ToolResult {
                ...
                step,
                call_id: call.call_id.clone(),
                message: dsh_llm::ToolResultMessage {
