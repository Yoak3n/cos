...
use dsh_core::{Context, CoreError, CoreResult, Service};
use dsh_session::ToolError;
...
impl ToolRegistry {
    ...
    pub async fn execute(&self, ctx: &Context, run: &ToolRun) -> ToolOutcome {
        ...
        let pre: CoreResult<PreDecision> = ctx
            .target(dsh_core::ScopeTarget::All)
        ...
        ctx.target(dsh_core::ScopeTarget::All)
