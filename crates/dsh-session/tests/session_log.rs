...
fn sample_log() -> Session {
    ...
    session.append_at(
        SessionEventData::AssistantChunk {
            turn: 1,
            step: 1,
            chunk: dsh_llm::StreamChunk::text("你"),
        },
        102,
    );
    session.append_at(
        SessionEventData::AssistantChunk {
            turn: 1,
            step: 1,
            chunk: dsh_llm::StreamChunk::text("好"),
        },
        103,
    );
    session.append_at(
...
fn version_mismatch_is_rejected() {
    let path =
        std::env::temp_dir().join(format!("dsh-session-version-{}.jsonl", std::process::id()));
...
fn chunk_delta_roundtrips_through_json() {
    let chunk = dsh_llm::StreamChunk::text("字");
    let value = serde_json::to_value(&chunk).unwrap();
    let back: dsh_llm::StreamChunk = serde_json::from_value(value).unwrap();
