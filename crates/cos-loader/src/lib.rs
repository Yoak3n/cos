//! cos-loader —— cordis.yml 解析 → 工厂解析 → 拓扑排序 → 挂载（P2）。
//!
...
//! （§6 防返工项），B 形态只加一个 dlopen source（P8 已接入：yml `name` 以
//! `./` 或 `dlopen:` 开头 → [`dlopen::DlopenPlugin`] 运行期加载）。
//!
